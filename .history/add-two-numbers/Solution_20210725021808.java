...
class Solution {
    ...
    private static ListNode addTwoNumbers(ListNode node1, ListNode node2) {
        ...
        String[] summedConcatenatedDigitsSplit = new StringBuilder(intToString(summedConcatenatedDigitsSplit)).toString()
                .split("");

        ListNode resultantNode = new ListNode();

        for (int i = summedConcatenatedDigitsSplit.length - 1; i > -1; --i) {
            ListNode node = new ListNode(reversedSummedConcatenatedDigits[i]);
            resultantNode.val = stringToIntsummedConcatenatedDigitss[i]);
