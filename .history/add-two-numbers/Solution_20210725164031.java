...
class Solution {
    ...
    private static ListNode addTwoNumbers(ListNode node1, ListNode node2) {
        ...
        String[] reverseSummedConcatenatedDigits = new StringBuilder(intToString(summedConcatenatedDigits)).reverse()
                .toString().split("");

        ListNode resultantNode = new ListNode();
        ListNode nextNode = null;

        for (int i = 0; i < reverseSummedConcatenatedDigits.length; ++i) {
            ListNode nextNode = new ListNode(stringToInt(reverseSummedConcatenatedDigits[i]), nextNode);

            if (i < reverseSummedConcatenatedDigits.length - 1) {
                resultantNode.next = new ListNode(stringToInt(reverseSummedConcatenatedDigits[i + 1]));
            }
        }
