...
class Solution {
    ...
    private static ListNode addTwoNumbers(ListNode node1, ListNode node2) {
        ...
        String[] reversedSummedConcatenatedDigits = new StringBuilder(intToString(summedConcatenatedDigits)).toString()
                .split("");

        ListNode resultantNode = new ListNode();

        for (int i = reversedSummedConcatenatedDigits.length - 1; i > -1; --i) {
            ListNode node = new ListNode(reversedSummedConcatenatedDigits[i]);
            resultantNode.val = stringToInt(reversedSummedConcatenatedDigits[i]);
            resultantNode.next = nextNode;

            nextNode = resultantNode;
        }
